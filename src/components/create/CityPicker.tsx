...
import type { CityResult } from "@/app/api/cities/search/route";
      ...
      {query.trim().length >= 2 && (
        <ul className="chat-city-results" role="listbox">
          {loading ? (
            ...
            <li className="chat-city-empty">No matches — try a fuller name.</li>
          ) : (
            results.map((r, i) => (
              <li
                key={`${r.city}-${r.lat}-${r.lng}`}
                role="option"
              ...
              >
                <span className="chat-city-name">{r.city}</span>
                <span className="chat-city-meta">
